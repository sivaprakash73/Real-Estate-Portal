...
const BACKUP_COLLECTIONS = ['properties', 'agents', 'leads'];
...
// ---------- Backup ----------
// A backup is deliberately kept as plain JSON so it can be downloaded, stored
// safely and restored without any third-party service.

function createBackup() {
  return {
    schemaVersion: 1,
    application: 'Vetrivelan Realty',
    exportedAt: new Date().toISOString(),
    data: Object.fromEntries(BACKUP_COLLECTIONS.map((name) => [name, read(name)])),
  };
}
...
function restoreBackup(backup) {
  ...
  if (!data || typeof data !== 'object') {
    throw new Error('This file is not a valid Vetrivelan Realty backup.');
  }

  for (const name of BACKUP_COLLECTIONS) {
    if (!Array.isArray(data[name])) {
      throw new Error(`The backup is missing a valid ${name} collection.`);
    }
  }

  // Validate the entire snapshot before writing so an invalid file never
  // leaves the local store half-restored.
  for (const name of BACKUP_COLLECTIONS) write(name, data[name]);
